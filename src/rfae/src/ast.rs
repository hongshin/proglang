...
pub enum Expr {
    ...
    Def(String, String, Box<Expr>, Box<Expr>),
...
pub enum Value {
    NumV(i32),
    CloV(String, Box<Expr>, Rc<RefCell<BTreeMap<String,Value>>>),
}
	...
	fn fmt (&self, f: &mut fmt::Formatter) -> fmt::Result {
		match self {
			...
			Expr::Use(id) => write!(f, "{}", id),
                        ...
                        Expr::Def(fun, x, e1, e2) => write!(f, "(def {}({})={} in {})", fun, x, e1, e2),
                        Expr::If0(ec, et, ef) => write!(f, "(if0 {} {} {})", ec, et, ef),
		}
	}
}
